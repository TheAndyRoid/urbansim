...
public class InterfaceTest extends TestCase {
	...
	public class C implements SomeInterface {
		public void doStuff() throws SuspendExecution {
			/*
			 * float time = 0f; float seconds = .8f; do { float t = .06667f;
			 * System.out.println("time = " + time + " " + (time + t)); // time
			 * = StrictMath.min(time + t, seconds); time = time + t;
			 * System.out.println("seconds = " + seconds + " | time = " + time +
			 * " | t = " + t); System.out.println("this = " + this);
			 * 
			 * System.out.println("time just not after = " + time);
			 * Coroutine.yield(); System.out.println("time after = " + time); }
			 * while (time < seconds); System.out.println("1 = " + 1);
			 */
		}
	}
	...
	public void testSuspend() {
		...
		Coroutine co = new Coroutine(new CoroutineProto() {
			public final void coExecute() throws SuspendExecution {
				...
				// TypeInterpreter
				// SomeInterface i = System.currentTimeMillis() > 0 ? new C() :
				// new C2();
